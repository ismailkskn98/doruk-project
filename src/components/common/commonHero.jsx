...
export default function CommonHero({ video, image, alt, title, lightTitle, subtitle, description, sideInfo = [], imageClassName = '', titleLightReverse = false, isDesilightPage = false }) {
  return (
    <main className="w-full fluid gridContainer">
      <MotionLeftView className="fluid gridContainer w-full overflow-hidden">
        {video ? (
          <section className="w-full flex items-center justify-center">
            {/* <div className="relative w-full min-[1500px]:aspect-1360/800 min-h-[200px] sm:min-h-80 max-h-[800px] overflow-hidden">
          ...
          <div className="relative w-full min-h-[200px] max-h-[800px] aspect-1360/800 min-[93.75rem]:fluid overflow-hidden min-[93.75rem]:px-12.5">
            <div className="relative w-full min-h-[200px] max-h-[800px] aspect-1360/800 min-[93.75rem]:fluid overflow-hidden">
      ...
      <MotionRightView className="w-full flex items-start justify-between gap-[clamp(14px,4vw,50px)] font-helvetica-neue py-[clamp(28px,4vw,50px)]">
        <article className="flex flex-col items-start gap-[clamp(14px,4vw,50px)]">
          <div className={`flex flex-col items-start gap-0 lg:gap-2.5 ${isDesilightPage ? '' : 'uppercase'}`}>
            <h2 className="font-bold text-[clamp(24px,4vw,32px)] leading-5 sm:leading-6 md:leading-7 text-nowrap">
              {!titleLightReverse ? (
                <>
                  {title} {lightTitle && <span className="font-light uppercase">{lightTitle}</span>}
                </>
              ) : (
                <>
                  {lightTitle && <span className="font-light uppercase">{lightTitle}</span>} {title}
                </>
              )}
            </h2>
