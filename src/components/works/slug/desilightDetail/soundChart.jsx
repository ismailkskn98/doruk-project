...
export default function SoundChart({ data = defaultData, yTicks = [], yTickFormatter, domain, barSize = 28 }) {
  const values = data.map((item) => item.value);
  const minValue = Math.min(...values);
  const maxValue = Math.max(...values);

  const calculatedDomain = domain || [Math.max(0, minValue - 10), maxValue + 10];
    ...
    <div className="w-full max-w-11/12 h-65.5">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 5, right: 15, left: -15, bottom: 5 }}>
          {yTicks.map((tick) => (
            <ReferenceLine key={tick} y={tick} stroke="#444" strokeDasharray="2 2" strokeWidth={0.5} />
          ))}

          <XAxis dataKey="name" tick={<CustomXAxisTick />} tickLine={false} axisLine={{ stroke: '#222', strokeWidth: 1 }} interval={0} />

          <YAxis
            ticks={yTicks}
            interval={0}
            domain={calculatedDomain}
            tick={(props) => <CustomYAxisTick {...props} formatter={yTickFormatter} />}
            tickLine={false}
